...
export function parseConfig(basePath: string): ParseConfigResult {
  const configFileName = ts.findConfigFile(basePath, ts.sys.fileExists, "tsconfig.json");
  if (!configFileName) {
  ...
  if (error) {
    throw new Error(`Error parsing TS config file ${configFileName}: ${error.messageText}`);
  }
